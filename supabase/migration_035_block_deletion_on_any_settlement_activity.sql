-- ============================================================
-- Migration 035: Block debt-linked expense deletion on any settlement activity
-- Run this in the Supabase SQL editor after migration 034.
-- ============================================================

-- Final deletion rule:
-- debt exists only -> deletion allowed
-- any settlement/payment row exists -> deletion blocked
...
AS $$
  ...
  all_obligation_ids AS (
    ...
    UNION
    SELECT peer.id
    FROM linked_obligations lo
    JOIN personal_obligations peer
      ON peer.relationship_id = lo.relationship_id
    WHERE lo.relationship_id IS NOT NULL
  ),
  relationship_ids AS (
  ...
  SELECT
    settled_obligations.settlement_id,
    settled_obligations.obligation_id,
    settled_obligations.status,
    settled_obligations.activity_reason
  FROM settled_obligations
  ...
  SELECT
    ...
    CASE
      ...
      WHEN pos.status = 'recalled' THEN 'settlement_history'
  ...
  FROM personal_obligation_settlements pos
  WHERE pos.id IN (SELECT id FROM directly_linked_settlements)
     OR pos.counterparty_settlement_id IN (SELECT id FROM directly_linked_settlements)
  ORDER BY settlement_id NULLS LAST
  LIMIT 1;
$$;
...
BEGIN
  ...
  IF FOUND THEN
    ...
    RAISE EXCEPTION 'This expense cannot be deleted because payment or settlement activity already exists.'
      USING DETAIL = format(
...
BEGIN
  IF EXISTS (
    ...
    RAISE LOG 'Obligation % blocked from deletion because settlement activity exists', OLD.id;
    RAISE EXCEPTION 'This expense cannot be deleted because payment or settlement activity already exists.';
  END IF;
