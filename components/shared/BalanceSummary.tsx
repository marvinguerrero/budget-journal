...
interface Props {
  ...
  onSettle?: (balance: NetBalance) => void
}

export function BalanceSummary({ balances, currentUserId, onSettle }: Props) {
      ...
      <div className="space-y-2">
        {myBalances.map((b, i) => {
          ...
          return (
            ...
            >
              <span className={`text-sm font-medium ${iOwe ? 'text-amber-700 dark:text-amber-400' : 'text-emerald-700 dark:text-emerald-400'}`}>
              ...
              <span className={`ml-auto font-bold tabular-nums text-sm ${iOwe ? 'text-amber-700 dark:text-amber-400' : 'text-emerald-700 dark:text-emerald-400'}`}>
              ...
              {iOwe && onSettle && (
                <button
                  type="button"
                  onClick={() => onSettle(b)}
                  className="ml-1 px-2.5 py-1 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:opacity-90 transition-opacity flex-shrink-0"
                >
                  Settle
                </button>
              )}
